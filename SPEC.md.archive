...
- ⚠️ **リクエストボディは全体をメモリに読み込む**
  - **レスポンスの選択**:
    - `:body` - ファイル全体をメモリに読み込み（小〜中規模ファイル用）
    - `:body-file` - ストリーミング配信（大きなファイル用、ファイルサイズ制限なし）
  - **静的ファイル配信**（`server/static-file`）は自動的に`:body-file`を使用してストリーミング配信

**メモリ使用量の例**:
- 小さいファイル（`:body`推奨）: GET /logo.png (100KB) × 10同時リクエスト = 1MB メモリ
- 中規模ファイル（`:body`可）: GET /document.pdf (5MB) × 10同時リクエスト = 50MB メモリ
- 大きいファイル（`:body-file`推奨）: GET /video.mp4 (50MB) × 10同時リクエスト = 約80KB メモリ（8KB × 10リクエスト、ストリーミング）

**ストリーミング配信**（✅ 実装済み）:

大きなファイル（動画、大きなPDF等）を**メモリ効率的に配信**できます。`:body-file`キーを使用することで、ファイル全体をメモリに読み込まずにチャンク単位（8KB）でストリーミング配信されます。

```qi
;; 大きなファイルをストリーミング配信
(defn video-handler [req]
  {:status 200
   :body-file "/path/to/large-video.mp4"  ; ストリーミング配信
   :headers {:content-type "video/mp4"}})

;; 静的ファイル配信も自動的にストリーミング対応
(server/static-file "./public")
```

**技術詳細**:
- `tokio::fs::File` + `ReaderStream` による非同期ファイル読み込み
- 8KBチャンク単位でストリーミング
- ファイルサイズの制限なし（以前の10MB制限を撤廃）
- メモリ使用量を大幅に削減
