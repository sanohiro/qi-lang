...
**ポイント**:
- マップやベクタのペアを扱うときは`[[k v]]`パターンが便利
- 関数パラメータでDestructuringすると、コードが簡潔になります
...
デフォルトでは`|>`は値を**最後の引数**として渡しますが、`_`を使うと**任意の位置**に挿入できます：
...
`apply`はリスト（またはベクタ）の要素を関数の引数として展開します。
...
(apply operation numbers)
; => 60
```

### reduceとの違い

`apply`と`reduce`は似た結果を出すことがありますが、用途が異なります：
...
; applyは関数が可変長引数を受け付ける場合に便利
; reduceは初期値を指定したり、累積的な計算に便利
(reduce + 100 [1 2 3])
; => 106（初期値100から開始）
```
...
- **パイプライン**: |> でデータフロー
- **`_`プレースホルダー**: パイプラインで任意の位置に値を挿入
- **Destructuring**: 関数パラメータでベクタを分解
- **部分適用**: partial
- **関数合成**: comp
- **apply**: リストを引数として展開
...
**新機能のハイライト**:
- `fn`、`defn`、`let`でDestructuring（ベクタの分解）がサポートされました
- パイプライン演算子`|>`で`_`プレースホルダーが使えるようになりました
