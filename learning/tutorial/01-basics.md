...
(format-kv ["name" "Alice"])  ; => "name=Alice"
```

**ポイント**:
- `[[x y]]`のように書くと、ベクタを要素に分解できます
- `map`などの高階関数で非常に便利です
...
**ポイント**:
- `[x y]`のような分解パターンで、ベクタの要素を個別の変数に束縛できます
- マップやリストの要素を扱うときに便利です
- `fn`や`defn`と同じDestructuring構文が使えます
...
**ポイント**:
- 無名関数でもDestructuringが使えます
- `map`などで複雑なデータを処理するときに便利です
...
- **Destructuring**: `fn`、`defn`、`let`でベクタの分解が可能
- **演算**: 算術、比較、論理
- **文字列**: 連結、操作、補間（f-string）
- **スコープ**: `let`でローカル変数
