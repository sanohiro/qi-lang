...
**マクロ**: ✅ `mac` `quasiquote` `unquote` `unquote-splice`（実装済み）、🚧 `uvar` `variable` `vmark`（未実装）
...
### 実装状況サマリー
- ✅ **コア機能（約65%完成）**: 8つの特殊形式、パイプライン、パターンマッチング、モジュールシステム基盤、loop/recur、マクロシステム
- ✅ **組み込み関数（66個）**: 算術、比較、リスト、文字列、マップ、述語、論理、高階関数、エラー処理、状態管理
- ✅ **データ構造**: nil、bool、整数、浮動小数点、文字列、シンボル、キーワード、リスト、ベクタ、マップ、関数、アトム
- ✅ **メタプログラミング**: マクロシステム（mac、quasiquote、unquote、unquote-splice）
- ✅ **文字列**: f-string補間
- ✅ **エラー処理**: try/error/defer完全実装（エラー時もdeferが実行される）
- ✅ **状態管理**: atom/deref/swap!/reset!完全実装
- 🚧 **未実装の重要機能**: 並行処理、@（derefの短縮形）、uvar/variable（マクロの衛生性）、標準モジュール群（str/csv/regex/http等）
