...
**⚠️ 絶対にこれを守れ（Rust実装でもQiコード実装でも同じ）**:
- **Rust実装**: 標準ライブラリ関数を自作する前に、`docs/spec/`と`docs/spec/FUNCTION-INDEX.md`で既存の関数を確認すること
- **Qiコード実装**: 標準ライブラリやexamplesのQiコードを書くときも、同様に既存関数を確認すること
...
- ✅ **既存関数の再実装**: 既に実装されている標準関数（例: `map/assoc-in`, `get-in`, `reduce`）を自作してしまう
...
#### 実装前のチェックリスト（Rust実装・Qiコード実装共通）

新しい機能を実装する前に、以下を確認すること：

1. **標準ライブラリ関数を確認（最優先）**
   ```bash
   # FUNCTION-INDEX.mdで既存関数を確認
   cat docs/spec/FUNCTION-INDEX.md

   # 特定の関数を検索
   rg "assoc-in\|get-in\|update-in\|reduce\|map" docs/spec/
   ```
...
3. **Qiコード実装時の追加チェック**
   ```bash
   # 既存のexamplesやstdで同じパターンがないか確認
   rg "関数名やパターン" examples/ std/

   # サンプルコードでは特にベストプラクティスを示すこと
   # ヘルパー関数を自作せず、標準ライブラリを使う模範例を示すべき
   ```

3. **docs/spec/ の該当セクションを読む**
   - データ構造関連 → `docs/spec/06-data-structures.md`
