...
## VSCodeデバッグサポート（将来実装予定）

将来的には、VSCodeの統合デバッガをサポートする予定です：
...
現在のバージョンでは、デバッガは常に利用可能ですが、多くの機能はデフォルトで無効になっています。将来的には、起動オプションでデバッガを有効化する方式になる予定です：

```bash
# 将来の予定
qi --debug script.qi
qi --debug-port 5678 script.qi  # DAP経由でアタッチ可能
```
