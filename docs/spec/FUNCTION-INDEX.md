...
### validation - データ検証
詳細は[18-stdlib-validation.md](18-stdlib-validation.md)を参照。

- `validate` - スキーマベースのデータ検証

**スキーマオプション**:
- `:type` - データ型（string, integer, number, boolean, map, vector, list, keyword, symbol, nil, any）
- `:required` - 必須フィールド
- `:min-length`, `:max-length` - 文字列長
- `:pattern` - 正規表現パターン
- `:min`, `:max` - 数値範囲
- `:positive`, `:integer` - 数値制約
- `:min-items`, `:max-items` - コレクションサイズ
- `:fields` - ネストしたマップのフィールドスキーマ
