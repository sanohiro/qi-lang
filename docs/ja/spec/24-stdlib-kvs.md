...
- [kvs/close - 接続をクローズ](#kvsclose---接続をクローズ)
- [基本操作](#基本操作)
- [数値操作](#数値操作)
- [リスト操作](#リスト操作)
- [ハッシュ操作](#ハッシュ操作)
...
- **リソース解放**: `kvs/close`で明示的に接続をクローズ可能

---

## kvs/close - 接続をクローズ

**KVS接続をクローズし、リソースを解放します。**
...
- **長時間実行するアプリケーション**: 接続リークを防ぐため、不要になった接続は`kvs/close`でクローズしてください
- **短命なスクリプト**: スクリプト終了時に自動的にリソースは解放されるため、明示的なクローズは不要です
- **接続プール**: 同じURLへの接続は内部でプールされますが、`kvs/close`は特定の接続IDのみをクローズします
