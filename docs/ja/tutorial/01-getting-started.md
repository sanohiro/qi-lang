...
Qiの**REPL**（対話的実行環境）は、コードを即座に試せる強力な開発ツールです。
...
**REPL の主な機能:**
- 🎨 **シンタックスハイライト** - キーワードや演算子が色分けされます
- 📝 **タブ補完** - 関数名や変数名をTabキーで補完
- 📋 **結果ラベル** - 過去の結果を `$1`, `$2` で参照可能
- ⏱️ **実行時間表示** - 遅い処理の時間を自動表示
- 🔄 **ホットリロード** - ファイル監視で自動再読み込み
...
開発中のファイルを監視して、保存するたびに自動で再読み込み：

```qi
qi:16> :watch src/lib.qi
Watching: src/lib.qi

# エディタで src/lib.qi を編集して保存すると...
...
マクロは `~/.qi/macros` に保存され、次回起動時も使えます。
