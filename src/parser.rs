...
impl Parser {
    ...
    fn error_with_line(&self, key: MsgKey, args: &[&str]) -> String {
        ...
        if let Some(span) = self.current_span() {
            if span.line != 0 || span.column != 0 {
                ...
                return format!("{}:{}:{}: {}", file_name, span.line, span.column, base_msg);
            }
        }
    ...
    fn parse_primary(&mut self) -> Result<Expr, String> {
        match self.current() {
            Some(Token::Nil) => {
                // SAFETY: current()がSomeであることを確認済み
                let span = self
                    .current_span()
                    .copied()
    ...
    fn parse_defn_internal(&mut self, is_private: bool, keyword: &str) -> Result<Expr, String> {
        ...
        let body = Box::new(if body_exprs.len() == 1 {
            // SAFETY: len() == 1 をチェック済み
            body_exprs
                .into_iter()
                .next()
                .expect("body should have exactly one expression")
        } else {
            Expr::Do {
                exprs: body_exprs,
                span: start_span,
